...
def function_voltage(voltage, new_dv_dt_value, time_step): #injectiontime
    function_voltage = (voltage + new_dv_dt_value * time_step)
    # if the last element of new_injectiontime is 0 then that means there is no voltage
...
def dv_dt(cap, res, current, new_voltage, new_injectiontime):
    function_dv_dt = ((1 / (cap * res)) * (res * current - new_voltage) * new_injectiontime)
    new_dv_dt_value.append(function_dv_dt)


def _hh_m(m, new_function_m_dot, new_function_m_infinity, new_injectiontime):
    function_hh_m = ((m * new_function_m_dot+ new_function_m_infinity) * new_injectiontime)
    hh_m.append(function_hh_m)

def _hh_n(n,new_function_n_dot, new_function_n_infinity, new_injectiontime):
    function_hh_n = ((n * new_function_n_dot - new_function_n_infinity) *new_injectiontime)
    hh_n.append(function_hh_n)

def _hh_h (h, new_function_h_dot, new_function_h_infinity, new_injectiontime):
    function_hh_h = ((h * new_function_h_dot / new_function_h_infinity) * new_injectiontime)
...
def fx_ina(gna,hh_m, hh_h):
    ina = gna * pow(hh_m, 3.0) * hh_h
    new_ina.append(ina)


def fx_ik(gk, hh_n, new_voltage, ek, new_time):
    ik = (gk * pow(hh_n, 4) * (new_voltage - ek) * new_time)
    new_ik.append(ik)

def fx_il(gl, new_voltage, el, new_time):
    il = ((gl * (new_voltage - el) * new_time) )
    new_il.append(il)
...
while initial_time < stop_time:
    ...
    dv_dt(cap, res, current, new_voltage[-1], new_injectiontime[-1])
  ...
  #  fx_ina(gna, hh_m[-1], new_voltage[-1], ena, hh_h[-1], new_time[-1])
    fx_ina(gna, hh_m[-1], hh_h[-1])
    fx_ik(gk, hh_n[-1], new_voltage[-1], ek, new_time[-1])
    _hh_m(m, new_function_m_dot[-1], new_function_m_infinity[-1],new_injectiontime[-1])
    _hh_n(n, new_function_n_dot[-1], new_function_n_infinity[-1], new_injectiontime[-1])
    _hh_h(h, new_function_h_dot[-1], new_function_h_infinity[-1], new_injectiontime[-1])


    function_voltage(new_voltage[-1], new_dv_dt_value[-1], time_step)  # calls
